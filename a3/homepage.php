  ...
  <!-- Referred w3 schools for the reference
	...
	<nav class="navbar navbar-expand-xl navbar-expand-lg navbar-expand-md navbar-expand-sm navbar-dark fixed-top">
	  <div class="header">
  	  <ul class="navbar-nav">
    		<li>
    		  <a class="navbar-brand" href="index.html"	>S I M P O S I O  <?php echo $_SESSION['login_user']; ?> </a>
    		</li>
  	  </ul>
...
<section>
  ...
  <input type="text" class="form-control" placeholder="Have a Question?" aria-label="user_ques" name="question">
...
$resultset = $obj->get_posts();


      foreach($resultset as $row){ ?>
      <div class="card mt-2">
      <div class="card-body">
      <h3><?php  echo  $row['question'].'</br>'; ?></h3>
      <div class="main">

      </div>
          <p><form action="single_view.php" method="GET">
          <button type="submit" class="btn btn-link" value="<?php echo $row['question_id'] ?>" name="id">Link</button>
          </form></p>
            </div>
            </div>
      <?php } ?>
